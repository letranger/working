...
\usepackage{CJKutf8}
\usepackage{xeCJK}
\setCJKmainfont{NotoSerifCJKtc-Medium}
\usepackage{CJK}
...
\title{\Huge{Python for Beginner}\\Random Examples}
\author{\huge{Yung Chin, Yen}}
\date{}
...
\chapter{\zht{Python簡介}}
\section{\zht{變數的概念}}
Another Test. \zht{中文，能不能秀變數和小馬?欸?這個作者的template簡直可以拿來出書了，屌打一堆出版軟體，如果能放進org-mode直接用就更讚了........}.  This is a test

\dfn{Limit of Sequence in $\bs{\bbR}$}{\zht{中文變數，馬的，可以欸...}\}Let $\{s_n\}$ be a sequence in $\bbR$. We say $$\lim_{n\to\infty}s_n=s$$ where $s\in\bbR$ if $\forall$ real numbers $\eps>0$ $\exists$ natural number $N$ such that for $n>N$ $$s-\eps<s_n<s+\eps\text{ i.e. }|s-s_n|<\eps$$}
\qs{}{\zht{這是個很難的題題}Is the set ${x-}$axis${\setminus\{\text{Origin}\}}$ a closed set}
\sol We have to take its complement and check whether that set is a open set i.e. if it is a union of open balls
\nt{\zht{這裡要特別注意}We will do topology in Normed Linear Space  (Mainly $\bbR^n$ and occasionally $\bbC^n$)using the language of Metric Space}
\clm{Topology}{}{\zht{非常好}Topology is cool}
\ex{\zht{這是很有用的練習題}Open Set and Close Set}{
	\begin{tabular}{rl}
		Open Set:   & $\bullet$ $\phi$                                              \\
