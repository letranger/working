...
ss = np.array([[1, 9], [2, 6], [3, 5], [4, 8],[6, 3], [7, 6], [8, 8], [9, 4]])
c1 = [5, 6]
c2 = [6, 9]
cx, cy = c1[0],  c1[1]
print(f'各點離中心點{cx},{cy}的距離')
for x,y  in ss:
    print(f'({x},{y}) to ({cx},{cy}): {np.sqrt(np.square(cx-x)+np.square(cy-y)):.2f}')
cx, cy = c2[0],  c2[1]
print(f'各點離中心點{cx},{cy}的距離')
for x,y  in ss:
    print(f'({x},{y}) to ({cx},{cy}): {np.sqrt(np.square(cx-x)+np.square(cy-y)):.2f}')
...
#nn = np.array([[5],[6]])
# 新的中心點:
print(f'新的中心點(orange): {c2}')
print(f'新的中心點(green): {c1}')
...
dist2 = []
c1xs, c1ys = np.repeat(c1[0],2), np.repeat(c1[1],2)
...
#+RESULTS:
#+begin_example
新的中心點(orange): [4.5 8.5]
新的中心點(green): [5.16666667 5.33333333]
5.166666666666667 5.333333333333333 2 6
5.166666666666667 5.333333333333333 3 5
5.166666666666667 5.333333333333333 4 8
5.166666666666667 5.333333333333333 6 3
5.166666666666667 5.333333333333333 7 6
5.166666666666667 5.333333333333333 9 4
5.166666666666667 5.333333333333333 1 9
5.166666666666667 5.333333333333333 8 8
--dist1--
3.24
2.19
2.91
...
ly1 = [cc[1], 3]
plt.plot(lx1, ly1, '--', color='green')
plt.annotate('4.87', (5.5, 4.2), textcoords="offset points", xytext=(5,5), ha='center')
# 新圖與狗
lx1 = [dc[0], 4.2]
ly1 = [dc[1], 3]
plt.plot(lx1, ly1, '--', color='orange')
plt.annotate('4.87', (3.5, 5), textcoords="offset points", xytext=(5,5), ha='center')
#for x,y  in zip(cats[0], cats[1]):
#    label = "貓({0},{1})".format(x,y)
#    plt.annotate(label, (x,y), textcoords="offset points", xytext=(0,10), ha='center')
#for x,y  in zip(dogs[0], dogs[1]):
...
plt.grid()
plt.savefig('images/hw20.png', dpi=300)
